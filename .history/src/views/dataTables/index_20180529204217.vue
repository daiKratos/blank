  ...
  methods: {
    ...
    handleCommit() {
      ...
      if (this.$refs.tabView.tableName.length === 2) {
        this.queryStatements = `select ${this.$refs.tabView.selectConditions} from product product join shop shop on product.shopRawId=shop.shopRawId  where ${
          this.whereConditions
        } limit 2,20`
      } else {
        this.queryStatements = `select ${this.$refs.tabView.selectConditions} from ${this.$refs.tabView.tableName[0]} where ${this.whereConditions} limit 2,20`
      }
    }
  },
  created() {
