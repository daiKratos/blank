  ...
  methods: {
    handleStatement() {
      ...
      if (this.$refs.tableView.tableName.length === 2) {
        const a = `select ${this.$refs.tabView.selectConditions} from productField join shopField on productField.shopRawId=shopField.shopRawId  where ${this.whereConditions}`
      }
