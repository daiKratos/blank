            ...
            Col(span='7')
              Select(style="width:300px", @on-change='handleCriterias')
                OptionGroup(v-if="selectedTabName.length > 1", v-for="(name,nameIndex) in selectedTabName", label='name', :key='nameIndex')
                  Option(v-for="(optionItem,itemIndex) in whereData.productField", :value='`${index}-${optionItem.column_name}-${optionItem.colum_type}`', :key="itemIndex") {{optionItem.column_name}}
                //- template(v-else)
