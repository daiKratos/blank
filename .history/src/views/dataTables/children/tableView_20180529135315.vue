    ...
    .field-con
      Select(v-model="field", multiple, style="width:300px", @on-change='handleOption', not-found-text='请选择表名', clearable)
        OptionGroup(label='productField', v-if="tableName.length===2")
          Option(v-for="(item,index) in fields.productField", :value="`shopField-${item.column_name}`", :key="index") {{ item.column_name }}
        OptionGroup(label='shopField', v-if="tableName.length===2")
          Option(v-for="(item,index) in fields.shopField", :value="item.column_name", :key="index") {{ item.column_name }}
