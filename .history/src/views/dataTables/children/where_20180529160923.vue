            ...
            Col(span='7')
              Select(style="width:300px", @on-change='handleCriterias')
                template(v-if="selectedTabName.length>1")
                  OptionGroup(v-for="name in selectedTabName", label='name')
                    Option(v-for="item,itemIndex) in data[name]", :value='`${name}-${index}-${item.column_name}-${item.colum_type}`', :key="item.colum_type") {{item.column_name}}
