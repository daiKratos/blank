            ...
            Col(span='7')
              Select(style="width:300px", @on-change='handleCriterias')
                OptionGroup(v-if="selectedTabName.length > 1", v-for="(selectedName,nameIndex) in selectedTabName", :label='selectedName', :key='nameIndex')
                  Option(v-for="(optionItem,itemIndex) in whereData[selectedName+'Field']", :value='`${selectedTabName}-${index}-${optionItem.column_name}-${optionItem.colum_type}`', :key="itemIndex") {{optionItem.column_name}}
                Option(v-else, v-for="(item,itemIndex) in data", :value='${index}-${item.column_name}-${item.colum_type}`', :key="itemIndex") {{item.column_name}}
