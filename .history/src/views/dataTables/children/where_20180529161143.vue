          ...
          Row
            Col(span='7')
              Select(style="width:300px", @on-change='handleCriterias')
                template(v-if="selectedTabName.length>1")
                  OptionGroup(v-for="(name,nameIndex) in selectedTabName", label='name', :key='nameIndex')
                    Option(v-for="(item,itemIndex) in data[name]", :value='`${name}-${index}-${item.column_name}-${item.colum_type}`', :key="itemIndex") {{item.column_name}}
                template(v-else)
                  Option(v-for="(item,itemIndex) in data", :value='`${name}-${index}-${item.column_name}-${item.colum_type}`', :key="itemIndex") {{item.column_name}}
                //- OptionGroup(label='product')
                //-   Option(v-for="(field,fieldIndex) in productFields", :value='`product-${index}-${field.column_name}-${field.colum_type}`', :key="fieldIndex") {{ field.column_name }}
                //- OptionGroup(label='shop')
                //-   Option(v-for="(shop,shopIndex) in shopFields", :value='`shop-${index}-${shop.column_name}-${shop.colum_type}`', :key="shopIndex") {{ shop.column_name }}
