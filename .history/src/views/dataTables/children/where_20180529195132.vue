    ...
    .conditions-con
      Form(ref='confitions', :model='confitions')
        FormItem(v-for="(item,index) in confitions.items", :key="index")
          Row
            Col(span='7')
              Select(style="width:300px", @on-change='handleCriterias', :disabled='selectedTabName.length===0')
            ...
            Col(span='7', v-if="item.Remaining && item.showValue")
              Input(placeholder="请输入内容", v-model="item.value", @on-blur='handleValue(index)')
</template>
  ...
  methods: {
    ...
    handleCriterias(value) {
      ...
      if (queryParam.length > 3) {
        ...
        this.tabName = queryParam[0]
        this.field = queryParam[2]
        ...
        this.field = queryParam[1]
        setValue(queryParam[2])
      ...
      this.confitions.items[this.focusItem].queryCriteria = ''
      this.confitions.items[this.focusItem].value = ''
      this.confitions.items[this.focusItem].field = this.field
    },
    handleValue(index) {
      if (this.confitions.items[index].queryCriteria === ' not in ' || this.confitions.items[index].queryCriteria === ' in ') {
        const containsValue = this.confitions.items[index].value.split(',')
        var showValue = '('
        containsValue.forEach((item, index) => {
          if (index === containsValue.length - 1) {
            showValue = showValue + `'${item}'`
          } else {
            showValue = showValue + `'${item}',`
          }
        })
