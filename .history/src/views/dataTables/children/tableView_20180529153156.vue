  ...
  section.tableView
    .table-con
      CheckboxGroup(v-model="tableName", @on-change='handleFields')
        ...
        Checkbox(label='shop')
    .field-con
      Select(v-model="field", multiple, style="width:300px", @on-change='handleOption', not-found-text='请选择表名')
...
export default {
  ...
  mounted() {
    this.initialData = this.data
      "http://192.168.10.24:9200/_sql?sql=select product.commentCount,product.goodCommentRate,product.monthSale from product where product.commentCount>32 and product.isAvailable=false and product.productName!='34' limit 2,20"
  }
}
</script>
