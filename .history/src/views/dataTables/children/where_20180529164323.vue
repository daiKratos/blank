          ...
          Row
            Col(span='7')
              Select(style="width:300px", @on-change='handleCriterias')
                ...
                //- Option(v-else, v-for="(radioItem,radioItemIndex) in whereData", :value='${radioItem.colum_type}`', :key="radioItemIndex") {{radioItem.column_name}}
                Option(v-for="test in cityList", :value="test.value", :key="test.value") {{ test.label }}
...
export default {
  ...
  mounted() {
    ...
    console.log(this.selectedTabName.length)
    whereData
  }
}
</script>
