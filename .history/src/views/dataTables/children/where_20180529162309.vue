          ...
          Row
            Col(span='7')
              Select(style="width:300px", @on-change='handleCriterias')
                ...
                Option(v-for="(optionItem,itemIndex) in whereData.productField", :value='`${name}-${index}-${optionItem.column_name}-${optionItem.colum_type}`', :key="itemIndex") {{optionItem.column_name}}
                //- template(v-else)
