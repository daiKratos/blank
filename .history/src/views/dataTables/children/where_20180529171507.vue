          ...
          Row
            Col(span='7')
              Select(style="width:300px", @on-change='handleCriterias', :disabled='selectedTabName.length===0')
                ...
                Option(v-if="selectedTabName.length <= 1", v-for="(radioItem,radioIndex) in whereData", :value='`${radioItem.column_name}-${optionItem.colum_type}`', :key='radioIndex') {{radioItem.column_name}}
