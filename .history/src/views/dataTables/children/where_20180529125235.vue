...
export default {
  ...
  methods: {
    ...
    handleStatement() {
      ...
      const a = ''
      const b = this.confitions.items.map((item, index) => {
        if (typeOf(item.queryCriteria) === 'boolean') {
          return `${item.tabName}.${item.field}${item.queryCriteria}`
        }
        return `${item.tabName}.${item.field}${item.queryCriteria}${item.value}`
      })
      console.log(b)
      const c = b.join(` ${this.character} `)
      console.log(c)
      // var a = ''
