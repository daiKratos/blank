  ...
  methods: {
    ...
    handleStatement() {
      ...
      if (this.$refs.tabView.tableName.length === 2) {
        queryStatements = `select ${this.$refs.tabView.selectConditions} from product product join shop shop on product.shopRawId=shop.shopRawId  where ${this.whereConditions} limit 2,20`
      } else {
