...
export default {
  ...
  methods: {
    handleStatement() {
      ...
      if (this.confitions.items[0].field !== '') {
        const statementArr = this.confitions.items.map((item, index) => {
          return `${item.tabName}.${item.field}${item.queryCriteria}${item.value}`
        })
        this.whereConditions = statementArr.join(` ${this.character} `)
      }
      console.log(this.$refs.whereView.whereConditions)
    }
  },
  created() {
