<template lang="pug">
  .home-con
    ...
    .body-con
      ...
      whereView(:data='whereData',:selectedTabName='selectedTabName', ref='whereView')
    ...
    handleStatement() {
      if (this.$refs.whereView.confitions.items[0].field !== '') {
        const statementArr = this.$refs.whereView.confitions.items.map((item, index) => {
          ...
          else return `${item.field}${item.queryCriteria}${item.value}`
        })
