    ...
    handleStatement() {
      if (this.$refs.whereView.confitions.items[0].field !== '') {
        const statementArr = this.$refs.whereView.confitions.items.map((item, index) => {
          if (item.type === 'String' && !item.value.includes('(')) re1turn `${item.field}${item.queryCriteria}'${item.value}'`
      ...
      if (this.$refs.tabView.tableName.length === 2) {
        a = `select ${this.$refs.tabView.selectConditions} from product product join shop shop on product.shopRawId=shop.shopRawId  where ${this.whereConditions} limit 2,20`
      } else {
